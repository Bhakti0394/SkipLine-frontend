...
const Auth = () => {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:8080/api/admin/auth/start-login", {
        method: "POST",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (role === "customer" && mode === "login") {
        ...
        window.location.href = "http://localhost:5173/customer-dashboard/overview";
      }
      ...
      if (role === "admin" && mode === "signup") {
        ...
        const res = await fetch("http://localhost:8080/api/admin/auth/register", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <div className="auth-container">
            ...
            >
              <div className="main-message">
                <div className="sustainability-badge" style={{ marginTop: "20px" }}>
                  ...
                  <span className="badge-text">Sustainability First</span>
                </div>
                ...
                <p className="main-description">
                  Smart pre-ordering enables accurate demand planning and efficient
                  kitchen operations for your campus.
                </p>
              </div>
              ...
              <div className="feature-pills">
                {features.map((feature, index) => (
                  ...
                  >
                    <feature.icon className="pill-icon" />
                    <span className="pill-text">{feature.text}</span>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="stats-grid">
                <div className="stat-item">
                  <p className="stat-value stat-primary">30min</p>
                  <p className="stat-label">→ 0 queue time</p>
                </div>
                <div className="stat-item">
                  <p className="stat-value stat-accent">40%</p>
                  <p className="stat-label">less food waste</p>
                </div>
                <div className="stat-item">
                  <p className="stat-value">3x</p>
                  <p className="stat-label">faster service</p>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      >
        <div className="auth-form-wrapper">
          ...
          <form
            onSubmit={
              mode === "login" && role === "admin" && adminStep === "email"
                ? handleSendOTP
                : mode === "login" && role === "admin" && adminStep === "otp"
                ? handleVerifyAdmin
                : handleSubmit
            }
            className="auth-form"
          >
            ...
            {/* Password Fields - Customer Signup */}
            ...
            {shouldShowAdminFields && (
              <>
                ...
                <motion.div
                  ...
                  className="form-field"
                >
                  <label htmlFor="kitchenName" className="field-label">
                    Kitchen Name
                  </label>
                  <div className="input-wrapper">
            ...
            <button type="submit" disabled={isLoading} className="submit-button">
              {isLoading ? (
                ...
                <>
                  {mode === "login" && role === "admin" && adminStep === "email" ? (
                    <>Send Code <Send className="button-arrow" /></>
                  ) : mode === "login" && role === "admin" && adminStep === "otp" ? (
                    <>Verify & Login <ArrowRight className="button-arrow" /></>
                  ) : (
